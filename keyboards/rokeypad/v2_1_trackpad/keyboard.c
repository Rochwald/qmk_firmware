...
painter_font_handle_t  font16;     // font object
...
const char *shorten_keycode(uint16_t keycode) {
    switch (keycode) {
        ...
        case MS_BTN1:        return "MS_L";
        case MS_BTN2:        return "MS_R";
        case MS_BTN4:        return "<-";
        case MS_BTN5:        return "->";
        default:             break;
    }
...
void display_write_single_keycode(uint8_t layer, uint8_t row, uint8_t col, painter_font_handle_t font) {
    ...
    #ifdef VIA_ENABLE
        uint16_t keycode = dynamic_keymap_get_keycode(layer, row, col+1);
    #else
        uint16_t keycode = keymaps[layer][row][col+1];
    #endif
...
void display_write_trackpad(char *text, painter_font_handle_t font, uint8_t h, uint8_t s, uint8_t v) {
    int16_t text_height = font->line_height;
    int16_t text_width = qp_textwidth(font, text);
    qp_drawtext_recolor(display, ((LCD_WIDTH-1)/2 - text_width/2), (((LCD_HEIGHT-1)/2 - text_height/2) - 1 + 55), font, text, HSV_BLACK, h, s, v);
}
...
void display_task_kb(void) {
    ...
    if (current_layer != last_drawn_layer || force_redraw) {
        ...
        for (int16_t col = 0; col < GRID_COLS - 1; col++) {
            int16_t x = LEFT_OFFSET + (col + 1) * CELL_WIDTH -1;
            qp_line(display, x, TOP_OFFSET, x, LCD_HEIGHT - BOTTOM_OFFSET - 1, HSV_BLACK);
        }

        // draw horizontal lines
        for (int16_t row = 0; row < GRID_ROWS - 1; row++) {
            int16_t y = TOP_OFFSET + (row + 1) * CELL_HEIGHT - 1;
            qp_line(display, LEFT_OFFSET, y, LCD_WIDTH - RIGHT_OFFSET - 1, y, HSV_BLACK);
        }

        // draw rectangle for touch area indication
        qp_rect(display, 9, 110, 230, 230, HSV_BLACK, false);
        current_layer_color = (hsv_helper_t){HSV_AZURE};
        display_write_trackpad("Touch area", font20, current_layer_color.h, current_layer_color.s, current_layer_color.v);

        // draw layer info
        switch (get_highest_layer(layer_state)) {
            case 0:
                current_layer_color = (hsv_helper_t){HSV_RED};
                display_write_layer_name("Numbers", font20, current_layer_color.h, current_layer_color.s, current_layer_color.v);
                display_write_all_keycodes(0, font16);
                break;
            ...
            case 1:
                ...
                display_write_all_keycodes(1, font16);
                break;
...
void post_process_record_kb(uint16_t keycode, keyrecord_t *record) {  // is executed AFTER every keypress
    if (record->event.key.col == 0 || record->event.key.col == 5) {
        return;
    }

    uint8_t pressed_row = record->event.key.row;
    uint8_t pressed_col = record->event.key.col - 1;       // 1-0
    ...
    display_write_single_keycode(get_highest_layer(layer_state), pressed_row, pressed_col, font16);
