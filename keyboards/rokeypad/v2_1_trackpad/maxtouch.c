...
// TODO: These are HW specific, they are used for handling CPI calculations.
...
// scroll speed - bigger number means slower scrolling
#ifndef MXT_SCROLL_DIVISOR
#   define MXT_SCROLL_DIVISOR 8
#endif
...
bool pointing_device_driver_init(void) {
    ...
    i2c_init();
    i2c_status_t status = i2c_read_register16(MXT336UD_ADDRESS, MXT_REG_INFORMATION_BLOCK, (uint8_t *)&information,
    ...
    if (status == I2C_STATUS_SUCCESS) {
        ...
        for (int i = 0; i < information.num_objects; i++) {
            ...
            i2c_status_t status = i2c_read_register16(MXT336UD_ADDRESS, SWAP_BYTES(object_table_element_address),
    ...
    if (t2_encryption_status_address) {
        ...
        i2c_status_t status = i2c_read_register16(MXT336UD_ADDRESS, t2_encryption_status_address,
    ...
    if (t100_multiple_touch_touchscreen_address) {
        ...
        i2c_status_t status                 = i2c_read_register16(MXT336UD_ADDRESS, t100_multiple_touch_touchscreen_address,
        ...
        status                              = i2c_write_register16(MXT336UD_ADDRESS, t100_multiple_touch_touchscreen_address,
                                                (uint8_t *)&cfg, sizeof(mxt_touch_multiscreen_t100), MXT_I2C_TIMEOUT_MS);
        if (status != I2C_STATUS_SUCCESS) {
...
report_mouse_t pointing_device_driver_get_report(report_mouse_t mouse_report) {
    ...
    if (t44_message_count_address) {
        ...
        i2c_status_t status = i2c_read_register16(MXT336UD_ADDRESS, t44_message_count_address,
            (uint8_t *)&message_count, sizeof(mxt_message_count), MXT_I2C_TIMEOUT_MS);
        if (status == I2C_STATUS_SUCCESS) {
            for (int i = 0; i < message_count.count; i++) {
                mxt_message message = {};
                status              = i2c_read_register16(MXT336UD_ADDRESS, t5_message_processor_address,
                ...
                if (message.report_id == t100_first_report_id) {
                    ...
                    // If a finger is held in the same spot for MXT_TAP_AND_HOLD_TIME, send a button down.
